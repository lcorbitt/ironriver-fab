...
import { useContactInfoBanner } from "./hooks/useContactInfoBanner";
import type { SocialNetwork } from "./types";
import { getSocialLinksFromSite } from "./utils";
...
const SOCIAL_GLYPHS: Record<SocialNetwork, () => ReactElement> = {
  instagram: () => (
    <svg
      width="16"
      height="16"
  ...
  yelp: () => (
    <svg
      ...
      aria-hidden
    >
      <path d="M12.27 19.77a1.88 1.88 0 0 1-2.54-2.55l1.27-2.54 2.54 1.27zm0-15.54a1.88 1.88 0 0 0-2.54 2.54l1.27 2.54 2.54-1.27zm7.46 7.77a1.88 1.88 0 0 0-2.54-2.54l-2.54-1.27 1.27 2.54a1.88 1.88 0 0 0 2.54 2.54zm-15.53 0a1.88 1.88 0 0 1 2.54-2.54l2.54-1.27-1.27 2.54a1.88 1.88 0 0 1-2.54 2.54z" />
    </svg>
  ),
};

const SocialGlyph = ({ network }: { network: SocialNetwork }) =>
  SOCIAL_GLYPHS[network]();

const socialLabel = (network: SocialNetwork): string => {
  switch (network) {
    case "instagram":
      return "Instagram";
    case "facebook":
...
export const ContactInfoBanner = () => {
  const { isHidden } = useContactInfoBanner();
  const socials = getSocialLinksFromSite(siteConfig.socialLinks);
